...
export async function createNetlifyProject(
  config: ClientNetlifyConfig,
): Promise<NetlifyProjectCreationResult> {
  try {
...
export async function deleteNetlifyProject(
  ...
  try {
    console.log(
      `Attempting to delete Netlify project for subdomain: ${subdomain}`,
    );
